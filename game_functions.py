...
def check_keydown(event, ai_settings, screen, ship, bullets):  # Respond to keypresses
...
def check_keyup(event, ship):  # Respond to key releases
...
def check_play_button(ai_settings, screen, stats, sb, play_button, ship, aliens, bullets, mouse_x,
                      mouse_y):  # Start new game when player clicks the play button
...
def check_high_score(stats, sb): # Check to see if there's a new high score
...
def get_number_rows(ai_settings, ship_height,
                    alien_height):  # Determine the number of rows of aliens that fit on the screen
...
def create_fleet(ai_settings, screen, ship, aliens):  # Create a fleet of aliens
...
def check_fleet_edges(ai_settings, aliens):  # Respond appropriately if any aliens have reached an edge
...
def change_fleet_direction(ai_settings, aliens):  # Drop the entire fleet and change the fleet's direction
    for alien in aliens.sprites():
        alien.rect.y += ai_settings.fleet_drop_speed
    ai_settings.fleet_direction *= -1


def ship_hit(ai_settings, screen, stats, sb, ship, aliens, bullets):  # Respond to ship colliding with alien
...
def check_aliens_bottom(ai_settings, screen, stats, sb,  ship, aliens,
                        bullets):  # Check if aliens have reached bottom of screen
...
def update_aliens(ai_settings, screen, stats, sb, ship, aliens, bullets):  # Update the positions of all aliens in the fleet
